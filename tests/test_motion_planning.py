...
def test_move_to_shelf():
    ...
    base_half_extents = [shelf_width / 2, shelf_length / 2, shelf_base_height / 2]
    ...
    pose = (
        0,
        0,
        shelf_base_height / 2 + shelf_ceiling_height - shelf_ceiling_thickness / 2,
    )
    link_positions.append(pose)
    half_extents = [shelf_width / 2, shelf_length / 2, shelf_ceiling_thickness / 2]
    ...
    for x_sign in [-1, 1]:
        for y_sign in [-1, 1]:
            pose = (
                x_sign * (shelf_width - shelf_pole_girth) / 2,
    ...
    block_id = create_pybullet_block(
        color, half_extents, obj_mass, obj_friction, default_orn, physics_client_id
    )
    p.resetBasePositionAndOrientation(
        block_id,
    ...
    world_to_base_link = get_link_state(
        robot.robot_id, robot.end_effector_id, physics_client_id=physics_client_id
    ).com_pose
    base_link_to_world = np.r_[
        p.invertTransform(world_to_base_link[0], world_to_base_link[1])
    ]
    world_to_obj = np.r_[
    ...
    robot_state = (
        tuple(target_pose.position)
        + tuple(target_pose.orientation)
    ...
    z += offset_z
    robot_state = (x, y, z) + tuple(home_pose.orientation) + (robot.closed_fingers,)
    robot.reset_state(robot_state)
